...
class QRLAgent:
    def __init__(self, n_layers=2):
        ...
        # Initialize weights using a PennyLane utility function
        self.qlayer_weights = qml.StronglyEntanglingLayers.init_weights(n_layers=n_layers, n_wires=n_qubits, seed=42)
    ...
    def choose_action(self, state):
        ...
        exp_val = quantum_circuit(state, self.qlayer_weights)
        prob_action_0 = (1 + exp_val) / 2
        
        # Create a probability distribution and sample an action
        action_probs = torch.tensor([prob_action_0, 1 - prob_action_0])
